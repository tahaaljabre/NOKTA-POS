...
router.post('/login', (req, res) => {
  ...
  const permissions = typeof emp.permissions === 'string' ? JSON.parse(emp.permissions || '{}') : (emp.permissions || {});
  // max_discount: NULL في DB يُعامل كـ 100 (بلا حد للخصم)
  const maxDiscount = (emp.max_discount !== null && emp.max_discount !== undefined) ? emp.max_discount : 100;
  res.json({
    id: emp.id,
    name: emp.name,
    name_en: emp.name_en,
    ...
    phone: emp.phone || '',
    default_floor: emp.default_floor || 1,
...
router.get('/setup-status', (req, res) => {
  ...
  res.json({
    setup_required: !admin && !db.prepare("SELECT value FROM settings WHERE key='setup_complete'").get(),
    recovery_required: !admin && !!db.prepare("SELECT value FROM settings WHERE key='setup_complete'").get(),
    migration_required: !!admin && !admin.username
  });
});
...
router.post('/setup-admin', (req, res) => {
  ...
  if (!String(name || '').trim() || !/^[a-z0-9._-]{3,32}$/.test(cleanUsername) || !strongPassword)
    return res.status(400).json({ error: 'Use a name, a 3–32 character username, and a strong password of at least 6 characters.' });
  ...
  if (db.prepare("SELECT value FROM settings WHERE key='setup_complete'").get() || db.prepare('SELECT id FROM employees LIMIT 1').get())
    return res.status(409).json({ error: 'التهيئة مغلقة؛ يلزم استرجاع إداري محلي / Setup locked; local administrator recovery required' });
  ...
  try {
    const info = db.prepare('INSERT INTO employees (username, name, name_en, pin, role, permissions) VALUES (?, ?, ?, ?, ?, ?)').run(
      cleanUsername, String(name).trim(), 'Manager', require('../database/db').hashPin(password), 'admin', JSON.stringify(permissions)
    );
