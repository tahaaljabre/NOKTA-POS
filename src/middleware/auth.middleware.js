...
function getRequesterInfo(req) {
  ...
  try {
    const emp = db.prepare('SELECT id, name, name_en, role, permissions, default_floor, default_station, active, token_rev FROM employees WHERE id=? AND active=1').get(payload.sub);
