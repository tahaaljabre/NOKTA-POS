...
(async()=>{const running=await require('../server').startServer();try{
 const result=await new Promise((resolve,reject)=>https.get({hostname:'127.0.0.1',port:running.server.address().port,path:'/api/auth/setup-status',ca:fs.readFileSync(config.tlsCertPath),rejectUnauthorized:false},res=>{let text='';res.on('data',d=>text+=d);res.on('end',()=>resolve({status:res.statusCode,text}));}).on('error',reject));
